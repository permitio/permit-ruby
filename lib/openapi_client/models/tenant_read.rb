  ...
  class TenantRead
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::TenantRead` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::TenantRead`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'last_action_at')
        self.last_action_at = attributes[:'last_action_at']
      end
    ...
    def list_invalid_properties
      ...
      if @last_action_at.nil?
        invalid_properties.push('invalid value for "last_action_at", last_action_at cannot be nil.')
      end
    ...
    def hash
      [key, id, organization_id, project_id, environment_id, created_at, updated_at, last_action_at, name, description, attributes].hash
    end
