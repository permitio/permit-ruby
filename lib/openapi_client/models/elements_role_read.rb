  ...
  class ElementsRoleRead
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::ElementsRoleRead` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::ElementsRoleRead`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @permission_level.nil?
        invalid_properties.push('invalid value for "permission_level", permission_level cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @permission_level.nil?
      true
    end
    ...
    def hash
      [name, description, permissions, extends, key, id, organization_id, project_id, environment_id, created_at, updated_at, permission_level].hash
    end
