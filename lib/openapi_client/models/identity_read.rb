  ...
  class IdentityRead
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::IdentityRead` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::IdentityRead`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'auth0_info')
        self.auth0_info = attributes[:'auth0_info']
      end
    end
    ...
    def list_invalid_properties
      ...
      if @auth0_info.nil?
        invalid_properties.push('invalid value for "auth0_info", auth0_info cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @auth0_info.nil?
      true
    end
    ...
    def hash
      [user_id, provider, sub, email, email_verified, auth0_info].hash
    end
