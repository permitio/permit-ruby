  ...
  class InviteRead
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::InviteRead` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::InviteRead`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @invite_code.nil?
        invalid_properties.push('invalid value for "invite_code", invite_code cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @invite_code.nil?
    ...
    def hash
      [email, role, id, organization_id, invite_code, created_at, status, failed_reason].hash
    end
