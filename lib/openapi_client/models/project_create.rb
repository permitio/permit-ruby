...
module OpenapiClient
  class ProjectCreate
    ...
    attr_accessor :active_policy_repo_id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::ProjectCreate` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::ProjectCreate`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # Custom attribute writer method with validation
    # @param [Object] urn_namespace Value to be assigned
    def urn_namespace=(urn_namespace)
      pattern = Regexp.new(/[a-z0-9-]{2,}/)
      if !urn_namespace.nil? && urn_namespace !~ pattern
        fail ArgumentError, "invalid value for \"urn_namespace\", must conform to the pattern #{pattern}."
      end
    ...
    def hash
      [key, urn_namespace, name, description, settings, active_policy_repo_id].hash
    end
