  ...
  class DetailedAuditLog
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::DetailedAuditLog` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::DetailedAuditLog`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'pdp_config_id')
        self.pdp_config_id = attributes[:'pdp_config_id']
      end
    ...
    def list_invalid_properties
      ...
      if @pdp_config_id.nil?
        invalid_properties.push('invalid value for "pdp_config_id", pdp_config_id cannot be nil.')
      end
    ...
    def hash
      [id, raw_data, input, result, context, action, timestamp, query, user_key, user_email, user_name, resource_type, tenant, decision, reason, pdp_config_id, env_id, project_id, org_id, objects].hash
    end
